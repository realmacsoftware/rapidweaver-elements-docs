...
If you know a bit of HTML you can create your own custom Components for use in your projects. These can be anything from a simple html snippet to a fully blown Component using the [Elements API](/broken/pages/noSL4R031DjxQ4kWZZmW), with custom UI controls. [Learn more about Custom Components](./#custom-components).

Custom Components can only be shared via projects, the code is open and visible to anyone with that project. They are perfect for creating bespoke solutions for your project(s), or getting familiar with the [Elements API](/broken/pages/noSL4R031DjxQ4kWZZmW) before you migrate to building components like those built into Elements.

### Third-party Components

Third-party developers can use the [Elements API](/broken/pages/noSL4R031DjxQ4kWZZmW) to create full-blown Components that are on a par with the built-in Components — Yes, we use exactly the same API that is available to all users.

Third-party components will be [distributed via the RapidWeaver Elements Distribution Platform](/broken/pages/uvbMAs9RdSWyVN273D5d) as Element Packs (.elementspack). These packs are shipped as highly optimized, encrypted bundles. The code cannot be modified or edited by users or third parties (exactly like the built-in Components).
